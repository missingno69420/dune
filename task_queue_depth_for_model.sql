...
running_sums AS (
  SELECT
    time,
    model,
    SUM(delta) OVER (ORDER BY time) AS unsolved_tasks
  FROM events
),
time_model AS (
  SELECT ts.time AS ts_time, FROM_HEX('{{model_id}}') AS model
  FROM time_series ts
),
ranked AS (
  SELECT
    ...
    tm.model,
    rs.unsolved_tasks,
    ROW_NUMBER() OVER (PARTITION BY tm.ts_time ORDER BY rs.time DESC) AS rn
  FROM time_model tm
  LEFT JOIN running_sums rs ON rs.time <= tm.ts_time
)
SELECT
