...
-- FeesPaid: Fees distributed for the task
SELECT
'FeesPaid' AS kind,
p.block_time,
p.task_id,
...
'model_owner_fee: ' || CAST(p.model_owner_fee AS VARCHAR) ||
', treasury_fee: ' || CAST(p.treasury_total_fee AS VARCHAR) ||
...
-- RewardsPaid: Rewards distributed for the task
SELECT
'RewardsPaid' AS kind,
p.block_time,
p.task_id,
